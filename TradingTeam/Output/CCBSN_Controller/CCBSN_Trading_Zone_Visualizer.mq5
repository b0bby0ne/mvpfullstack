...
#property version   "3.210"
#property description "CCBSN M15 ATR20 + EMA23 Trading Zone Controller"
...
input group "02. Ownership & CCBSN Control"
...
const int    InpATRPeriod                 = 20;
const int    InpEMAPeriod                 = 23;
const double InpMaxAboveEMAPrice          = 20.0;
const double InpMinBelowEMAPrice          = 20.0;
...
const color InpChartBackgroundColor = clrWhite;
...
const color InpPanelBackgroundColor = clrWhite;
const color InpPanelBorderColor     = C'205,210,218';
const color InpPanelTextColor       = C'45,55,70';
...
const string InpTextPanelTitle      = "CCBSN CONTROLLER v3.21";
...
const string InpTextControl         = "Applied NC";
...
const int InpTimerSeconds           = 1;
const string InpCsvFileName         = "CCBSN_Trading_Zone_Events_v3_21.csv";
...
const string POLICY_ID      = "ccbsn-m15-atr20-ema23-gate";
const string POLICY_VERSION = "1.2.1-configurable-magic";
...
bool ValidateInputs()
  {
   ...
   if(InpATRPeriod < 1 || InpEMAPeriod < 1 || InpEnableConfirmBars < 1)
     {
      Print("CONFIG ERROR | Indicator periods and confirmation bars must be positive.");
      return false;
     }
   ...
   if(InpTimerSeconds < 1 || InpEMALineWidth < 1 || InpEMALineWidth > 5)
     {
...
void CreateEventMarker(const string eventType,
                       ...
                       const string tooltip)
  {
   ...
   ObjectSetInteger(0, name, OBJPROP_COLOR,
                    ColorWithOpacity(textColor, InpEventOpacityPercent));
...
void CreatePanel()
  {
   ...
   ObjectSetInteger(0, background, OBJPROP_XSIZE, 540);
   ObjectSetInteger(0, background, OBJPROP_YSIZE, 202);
   ObjectSetInteger(0, background, OBJPROP_BGCOLOR, InpPanelBackgroundColor);
...
void UpdatePanel()
  {
   ...
   SetPanelLabel("TITLE", 22,
                 InpTextPanelTitle + " | " + InpTextMode + " | " +
                 ControlModeToString(InpControlMode),
                 InpChartForegroundColor);
   SetPanelLabel("STATE", 42,
                 StringFormat("%s: %-10s  %s: %d/%d  %s: %s",
                              InpTextState,
                 ...
                 stateColor);
   SetPanelLabel("ATR", 62,
                 StringFormat("M15 Close: %s | ATR%d: %s / Min: %s",
                              FormatPrice(g_lastClose), InpATRPeriod,
                              FormatPrice(g_lastATR), FormatPrice(InpMinATRPrice)),
                 InpPanelTextColor);
   SetPanelLabel("EMA", 82,
                 StringFormat("EMA%d: %s | D=Close-EMA: %s | %s: %s",
                ...
                const bool historical)
  {
   ...
   if(FileSize(handle) == 0)
      FileWrite(handle, "policy", "version", "symbol", "event_time", "event",
                "state", "close", "atr", "min_atr", "ema", "distance",
   ...
   FileWrite(handle, POLICY_ID, POLICY_VERSION, _Symbol,
             ...
             FormatPrice(InpMinATRPrice), FormatPrice(g_lastEMA),
             FormatPrice(g_lastDistance), g_consecutivePassCount,
             ...
             g_commandCancelRequested ? "true" : "false", reason);
   FileFlush(handle);
   FileClose(handle);
  }
   ...
   if(distance >= 0.0 && distance <= InpMaxAboveEMAPrice)
     {
      ...
      reason = "M15_EMA23_ABOVE_WITHIN_20";
      return true;
     }
   if(distance < -InpMinBelowEMAPrice)
     {
      branch = ZONE_BRANCH_DEEP_BELOW_EMA;
      reason = "M15_EMA23_BELOW_MORE_THAN_20";
      return true;
     }

   if(distance < 0.0)
      reason = "M15_EMA23_BELOW_WITHIN_20";
   else
      reason = "M15_EMA23_ABOVE_MORE_THAN_20";
   return false;
  }
   ...
   if(!historical || InpDrawHistory)
     {
      ...
      CreateEventMarker("ON", startTime, startPrice - atrValue * 0.35,
                        InpTextZoneOn, zoneColor, tooltip);
   ...
   if(!historical || InpDrawHistory)
     {
      ...
      CreateEventMarker("OFF", endTime, endPrice + g_activeZoneATR * 0.35,
                        InpTextZoneOff, InpOffEventColor, tooltip);
...
int OnInit()
  {
   ...
   if(!EventSetTimer(InpTimerSeconds))
     {
      PrintFormat("INIT ERROR | EventSetTimer failed | error=%d", GetLastError());
      return INIT_FAILED;
     }
   CreatePanel();
   BuildHistoricalZones();
