//+------------------------------------------------------------------+
//|                  CCBSN_Trading_Zone_Controller_v2.mq5 v2.19      |
//|        Pine v1.11 policy + resilient New Cycle controller        |
//+------------------------------------------------------------------+
#property strict
#property copyright "TradingTeam"
#property version   "2.190"
#property description "CCBSN Controller v2 - MT5 port of TradingView policy v1.11"
#property description "Bear Drop Risk Lock, session gate, active-only consecutive RED."
...
input group "04. Bear Drop Protection"
...
input group "05. Active Zone Candle OFF"
...
input group "06. New Cycle Sessions (broker server time)"
...
input group "07. Confirmation & Bear Drop Recovery"
input int    InpEnableConfirmBars          = 2;
input int    InpRiskLockBars               = 2;
...
input group "08. Ownership & CCBSN Control"
...
input group "09. Display - Chart & Dashboard"
...
input group "10. Trading Zone History (Visual Only)"
...
input group "11. Display - Risk Lock History"
...
input group "12. Display - EMA History"
input bool   InpShowEMAOnChart        = true;
input int    InpEMADisplayBars        = 400;

input group "13. Display - Event History & Visibility"
...
input group "14. Text - Dashboard"
input string InpTextPanelTitle      = "CCBSN CONTROLLER v2.19";
...
input group "15. Chart Event Names - Editable"
...
const string InpCsvFileName         = "CCBSN_Trading_Zone_Events_v2_19.csv";
...
const string POLICY_ID      = "ccbsn-m15-pine-v1-11-controller";
const string POLICY_VERSION = "2.0.19-fast-ack-handshake";
...
bool EvaluateBearDrop(const MqlRates &bar)
  {
   ...
   g_lastLegacyBearDrop = InpEnableLegacyBearDrop && legacyReady &&
                          g_lastRelativeDrop >= InpMinRelativeDropPrice &&
   ...
   g_lastTwoBarBearDrop = InpEnableTwoBarBearDrop && twoBarReady &&
                          g_lastTwoBarDrop >= InpMinTwoBarDropPrice;
...
bool ValidateInputs()
  {
   ...
   if(InpEnableConfirmBars < 1 || InpEnableConfirmBars > 20)
      return ConfigurationError("InpEnableConfirmBars", IntegerToString(InpEnableConfirmBars), "1..20");
   if(InpMinATRPrice <= 0.0)
      return ConfigurationError("InpMinATRPrice", DoubleToString(InpMinATRPrice, 4), "> 0");
   if(InpMaxAboveEMAPrice < 0.0)
      return ConfigurationError("InpMaxAboveEMAPrice", DoubleToString(InpMaxAboveEMAPrice, 4), ">= 0");
   if(InpMinBelowEMAPrice < 0.0)
      return ConfigurationError("InpMinBelowEMAPrice", DoubleToString(InpMinBelowEMAPrice, 4), ">= 0");
   ...
   if(InpRiskLockBars < 1 || InpRiskLockBars > 96)
      return ConfigurationError("InpRiskLockBars", IntegerToString(InpRiskLockBars), "1..96");
...
void UpdatePanel()
  {
   ...
   SetPanelColumnLabel("RED_LOCK", true, 202,
                       StringFormat("RED=%d/%d | Lock=%d Rec=%s %d/%d",
...
void AuditEvent(const string eventType,
  ...
  {
   if(!historical)
      PrintFormat("CCBSN_V2 | %s | %s | state=%s session=%s | close=%s atr=%s ema=%s d=%s | bear=%s source=%s red=%d/%d pattern=%s body=%s/%s deny=%s sweep=%s overlap=%.1f%% fall=%s range=%s/%s reverse=%s body=%s/%s lock=%d recover=%s scob=%s | %s",
   ...
   if(FileSize(handle) == 0)
      FileWrite(handle, "policy", "version", "symbol", "event_time", "event",
                "state", "close", "atr", "atr_period", "min_atr", "ema",
                 ...
                 "decision_session", "active_session", "pass_count",
   ...
   FileWrite(handle, POLICY_ID, POLICY_VERSION, _Symbol,
             ...
             InpEMAPeriod, FormatPrice(g_lastDistance),
              ...
              SessionToString(g_activeSession), g_consecutivePassCount,
...
string FeatureTooltip(const datetime decisionTime, const string eventType, const string reason)
  {
   return StringFormat("%s\n%s | %s\nClose=%s EMA%d=%s D=%s\nATR%d=%s MinATR=%s"
                       ...
                       g_lastBullishSCOB ? "YES" : "NO", reason);
  }
...
void StartActiveZone(const datetime startTime,
                     ...
                     const bool historical)
  {
   ...
   string tooltip = FeatureTooltip(startTime, "SIMULATED " +
                                   InpEventNamePolicyAllow, reason);
   ...
   if(ShouldRenderHistoricalEvent(historical,
                                  InpShowPolicyAllowEvents))
     {
      ...
      CreateEventMarker("ON", startTime, startPrice - atrValue * 0.35,
                        InpEventNamePolicyAllow, tooltip);
     }
...
void EndActiveZone(const datetime endTime,
                   ...
                   const bool showChartEvent)
  {
   ...
   string tooltip = FeatureTooltip(endTime, "SIMULATED " +
...
void ProcessDecisionBar(const MqlRates &bar,
                        ...
                        const bool historical)
  {
   ...
   string baseReason = "";
   bool checklistPass = EvaluateChecklist(bar.close, atrValue, emaValue,
                                           branch, baseReason);
   if(checklistPass || baseReason != "M15_DATA_NOT_READY")
      PushFeatureSample(bar, emaValue);

   bool bearDropVeto = baseReason != "M15_DATA_NOT_READY" &&
                       EvaluateBearDrop(bar);
   if(baseReason == "M15_DATA_NOT_READY")
     {
   ...
   if(wasActive && barSession == g_activeSession)
      g_activeConsecutiveRedCount = bar.close < bar.open
                                   ? g_activeConsecutiveRedCount + 1 : 0;
   else
      g_activeConsecutiveRedCount = 0;

   bool consecutiveRedBlock = InpEnableConsecutiveRedBlock && wasActive &&
                               barSession == g_activeSession &&
                               g_activeConsecutiveRedCount >= InpConsecutiveRedBars;
   ResetBearishPatternSnapshot();
   ResetDenySnapshot();
   ResetReverseSnapshot();
   ...
   bool sessionChecklistPass = checklistPass &&
                               g_lastDecisionSession != POLICY_SESSION_OUTSIDE;
   bool effectivePass = sessionChecklistPass && !bearDropVeto &&
                          !denyBlock && !fallBlock && !reverseBlock &&
                          !bearishPatternBlock &&
                         !consecutiveRedBlock &&
                         !sessionExit;
   ...
   if(sessionExit)
     ...
     {
      reason = "M15_BEAR_DROP_" + g_lastBearDropSource + "_VETO";
       if(denyBlock)
          reason += "_AND_DENY";
       if(fallBlock)
          reason += "_AND_FALL";
       ...
       if(bearishPatternBlock)
         reason += "_AND_BEARISH_PATTERN_" + g_lastBearishPatternSource;
      if(consecutiveRedBlock)
         reason += "_AND_CONSECUTIVE_RED";
     }
   else if(denyBlock)
     {
       reason = "M15_DENY_UPTHRUST_REJECTION_POLICY_BLOCK";
       if(fallBlock)
          reason += "_AND_FALL";
       if(reverseBlock)
          reason += "_AND_REVERSE";
     ...
     {
      reason = "M15_FALL_3_BAR_CLUSTER_DOWNSIDE_BREAK_POLICY_BLOCK";
      if(reverseBlock)
         reason += "_AND_REVERSE";
      if(bearishPatternBlock)
         reason += "_AND_BEARISH_PATTERN_" + g_lastBearishPatternSource;
     ...
     {
      reason = "M15_REVERSE_PIN_THEN_LARGER_BEARISH_BODY_POLICY_BLOCK";
      if(bearishPatternBlock)
         reason += "_AND_BEARISH_PATTERN_" + g_lastBearishPatternSource;
      if(consecutiveRedBlock)
         reason += "_AND_CONSECUTIVE_RED";
   ...
   // An ACTIVE session always ends cleanly at its own boundary. The next
   // session must arm from the following decision, even if sessions touch.
   if(sessionExit)
     {
   ...
   // BEAR DROP is the only path into RISK LOCK. It has priority when a
   // consecutive RED block occurs on the same closed M15 candle.
   if(bearDropVeto)
     {
      ...
      StartOrRefreshRiskLock(bar, atrValue, decisionTime, reason, historical,
                             enteringRiskLock || newBearDropEpisode, wasActive);
      return;
     }

   // DENY confirms a failed upthrust on two closed M15 candles. It has
   // priority over generic bearish patterns but never creates RISK LOCK.
   if(denyBlock)
     {
      EndActiveZone(decisionTime, bar.close, reason, historical,
                    InpEventNameDeny + "\n" + InpEventNamePolicyBlock,
   ...
   // FALL is a bearish candle whose complete High-Low range is large enough
   // versus the prior three-bar cluster and closes below the cluster low.
   // It does not need to reach the cluster high. It is a Soft OFF only.
   if(fallBlock)
     {
      EndActiveZone(decisionTime, bar.close, reason, historical,
                    InpEventNameFall + "\n" + InpEventNamePolicyBlock,
   ...
   // REVERSE confirms an upper-rejection pin bar followed by a bearish
   // candle whose real body is strictly larger. It is a Soft OFF only.
   if(reverseBlock)
     {
      EndActiveZone(decisionTime, bar.close, reason, historical,
                    InpEventNameReverse + "\n" + InpEventNamePolicyBlock,
   ...
   // A confirmed bearish engulfing or bearish pin bar blocks New Cycle only
   // while ACTIVE. Like consecutive RED, it creates no RISK LOCK/cooldown.
   if(bearishPatternBlock)
     {
   ...
   // Consecutive RED is sampled only while New Cycle is ACTIVE. It blocks
   // New Cycle immediately but creates no cooldown and no recovery flow.
   if(consecutiveRedBlock)
     {
      EndActiveZone(decisionTime, bar.close, reason, historical,
      ...
      g_riskLockRemaining = 0;
      g_consecutiveRecoveryBars = 0;
      AuditEvent("CONSECUTIVE_RED_POLICY_BLOCK", decisionTime, reason, historical);
      return;
     }
   ...
   if(wasRiskLock)
     {
      ...
      // Recovery is intentionally evaluated only while the state snapshot at
      // the beginning of this decision was RISK_LOCK.
      ...
      g_lastPolicyRecoveryCandidate = effectivePass &&
         g_lastDistance >= atrValue * InpRecoveryBufferATR &&
         ...
         if(g_consecutiveRecoveryBars >= InpRecoveryBars)
           {
            ...
            g_consecutiveRecoveryBars = 0;
            g_consecutivePassCount = InpEnableConfirmBars > 1 ? 1 : 0;
   ...
   if(sessionChecklistPass)
     {
      if(g_state == VISUAL_STATE_ARMING &&
         g_armingSession != g_lastDecisionSession)
         g_consecutivePassCount = 0;
      if(g_consecutivePassCount == 0)
         g_armingSession = g_lastDecisionSession;
      g_consecutivePassCount++;
      g_state = VISUAL_STATE_ARMING;
      if(g_consecutivePassCount >= InpEnableConfirmBars)
        {
         StartActiveZone(decisionTime, bar.close, atrValue, branch,
                         reason, historical);
        }
      else
        {
         string armText = StringFormat("%s %d/%d", InpEventNameArm,
                                       g_consecutivePassCount,
                                       InpEnableConfirmBars);
   ...
   if(!g_configurationValid)
     {
      ...
      g_objectPrefix = "CCBSN_TZ_V2." + IntegerToString(ChartID()) + ".";
