//+------------------------------------------------------------------+
//|                    CC_Controller_M5_Ver4_1.mq5                    |
//|       CCBSN cycle control + integrated M5 runtime guard           |
//+------------------------------------------------------------------+
#property strict
#property copyright "TradingTeam"
#property version   "4.100"
#property description "CC Controller M5 Ver4.1 - integrated fail-closed runtime guard"
#property description "Portable identity, quote detection, startup barrier and New Cycle handshake."
...
input bool   InpAutoDetectQuoteDigits = true;
...
input long   InpExpectedAccountLogin  = 0;        // Required in integrated mode
input string InpExpectedAccountServer = "";      // Exact MT5 server name
...
input bool   InpForceSyncOnInit        = true;     // Integrated startup always discards cached ACK
input int    InpStartupPolicyHoldSeconds = 70;    // Remain OFF after aligned startup ACK
...
input string InpTextPanelTitle      = "CC CONTROLLER M5 | VER4.1 INTEGRATED";
...
input bool   InpEnableExternalMonitor  = true;
input string InpMonitorStatusFile      = "CCBSN\\cc_controller_m5_ver4_1_status.json";
...
const string InpCsvFileName         = "CC_Controller_M5_Ver4_1_Events.csv";
const bool InpKeepObjectsOnRemove   = false;
const ulong TICKET_STORAGE_BASE     = 1000000000;
const string MONITOR_SCHEMA_VERSION = "ccbsn-lite-monitor-status.v2";

const ENUM_TIMEFRAMES DECISION_TIMEFRAME = PERIOD_M5;
const string POLICY_ID      = "cc-controller-m5-ver4.1";
const string POLICY_VERSION = "4.1.0-integrated";
...
bool g_identityViolationLatched = false;
int g_effectiveQuoteDigits = 2;
...
ulong g_startupPolicyHoldUntilTick = 0;
...
ENUM_CCBSN_COMMAND DesiredCommand()
  {
   ...
   if(g_startupCycleBarrierActive ||
      (g_startupPolicyHoldUntilTick > 0 &&
       GetTickCount64() < g_startupPolicyHoldUntilTick))
...
string FormatPrice(const double value)
  {
   return DoubleToString(value, g_effectiveQuoteDigits);
  }
...
bool ValidateInputs()
  {
   ...
   g_effectiveQuoteDigits = InpAutoDetectQuoteDigits
                            ? _Digits : (int)InpXAUQuoteDigits;
   if(g_effectiveQuoteDigits != 2 && g_effectiveQuoteDigits != 3)
      return ConfigurationError("EffectiveQuoteDigits",
                                IntegerToString(g_effectiveQuoteDigits),
                                "2 or 3 for XAUUSD");
   if(InpExpectedSymbolPrefix != "" && StringFind(_Symbol, InpExpectedSymbolPrefix) != 0)
      return ConfigurationError("InpExpectedSymbolPrefix",
                                ...
                                "prefix of " + _Symbol);
    if(!InpAutoDetectQuoteDigits && _Digits != (int)InpXAUQuoteDigits)
       return ConfigurationError("InpXAUQuoteDigits",
                                 IntegerToString((int)InpXAUQuoteDigits),
                                 IntegerToString(_Digits) + " for " + _Symbol);
   if(InpExpectedAccountLogin <= 0)
      return ConfigurationError("InpExpectedAccountLogin",
                                IntegerToString(InpExpectedAccountLogin),
                                "> 0 and equal to the intended MT5 login");
   if(InpExpectedAccountServer == "")
      return ConfigurationError("InpExpectedAccountServer", "empty",
                                "exact MT5 account server name");
   long actualLogin = AccountInfoInteger(ACCOUNT_LOGIN);
   string actualServer = AccountInfoString(ACCOUNT_SERVER);
   if(actualLogin != InpExpectedAccountLogin)
      return ConfigurationError("AccountLogin",
                                IntegerToString(actualLogin),
                                IntegerToString(InpExpectedAccountLogin));
   if(actualServer != InpExpectedAccountServer)
      return ConfigurationError("AccountServer", actualServer,
   ...
   if(InpMonitorHeartbeatSeconds < 1 ||
      ...
      return ConfigurationError("InpMonitorHeartbeatSeconds",
                                ...
                                "1..60");
   if(InpStartupPolicyHoldSeconds < 65 ||
      InpStartupPolicyHoldSeconds > 300)
      return ConfigurationError("InpStartupPolicyHoldSeconds",
                                IntegerToString(InpStartupPolicyHoldSeconds),
                                "65..300");
...
bool EnforceRuntimeIdentity()
  {
   if(g_identityViolationLatched)
      return false;
   long actualLogin = AccountInfoInteger(ACCOUNT_LOGIN);
   string actualServer = AccountInfoString(ACCOUNT_SERVER);
   if(actualLogin == InpExpectedAccountLogin &&
      actualServer == InpExpectedAccountServer)
     {
      g_identityValid = true;
      return true;
     }
   ...
   g_identityValid = false;
   g_identityViolationLatched = true;
   g_startupCycleBarrierActive = true;
   g_startupPolicyHoldUntilTick = 0;
   g_controlState = CCBSN_CONTROL_ERROR;
   g_lastControlError = StringFormat(
      "IDENTITY_MISMATCH:%I64d@%s_EXPECTED_%I64d@%s",
      actualLogin, actualServer, InpExpectedAccountLogin,
      InpExpectedAccountServer);
   g_syncState = "IDENTITY SAFE MODE";
   g_lastSyncReason = g_lastControlError;
   g_monitorRuntimeState = "IDENTITY_SAFE_MODE";
   Print("IDENTITY FAIL-CLOSED | " + g_lastControlError);
   Alert("CC CONTROLLER IDENTITY MISMATCH: automatic control stopped. " +
         g_lastControlError);
   AuditEvent("IDENTITY_FAIL_CLOSED", TimeCurrent(),
...
void ReportPerformanceMetrics(const string context,
                              const bool forceReport)
  {
   ...
   PrintFormat("PERF CC CONTROLLER M5 VER4.1 | context=%s elapsed=%.1fs | ticks=%I64u timers=%I64u | policy=%I64u/%I64u | control_fast=%I64u idle=%I64u | visual=%I64u dashboard=%I64u snapshots=%I64u redraw=%I64u ema=%I64u/%I64u monitor=%I64u/%I64u",
   ...
   PrintFormat("PERF LATENCY CC CONTROLLER M5 VER4.1 | policy_avg=%.1fus max=%I64uus | control_avg=%.1fus max=%I64uus | visual_avg=%.1fus max=%I64uus",
...
void CreatePanel()
  {
   ...
   ObjectSetInteger(0, background, OBJPROP_YDISTANCE, 15);
   ObjectSetInteger(0, background, OBJPROP_XSIZE, 650);
   ObjectSetInteger(0, background, OBJPROP_YSIZE, 320);
   ObjectSetInteger(0, background, OBJPROP_BGCOLOR,
                    (long)InpDashboardBackgroundColor);
...
void UpdatePanel()
  {
   ...
   SetPanelColumnLabel("PERF_OBJECTS", true, 265,
                       StringFormat("Snap/EMA: %I64u | %I64u/%I64u",
                                    ...
                                    g_perfEMAAppends),
                        InpDashboardTextColor);
   color identityColor = g_identityValid && !g_identityViolationLatched
                         ? C'0,120,80' : C'190,50,60';
   SetPanelColumnLabel("IDENTITY", false, 285,
                       StringFormat("Identity: %I64d@%s",
                                    AccountInfoInteger(ACCOUNT_LOGIN),
                                    AccountInfoString(ACCOUNT_SERVER)),
                       identityColor);
   SetPanelColumnLabel("PORTABLE", true, 285,
                       StringFormat("Guard: %s | Digits=%d",
                                    g_identityValid &&
                                    !g_identityViolationLatched
                                    ? "READY" : "BLOCKED",
                                    g_effectiveQuoteDigits),
                       identityColor);
  }
   ...
   if(ackAligned)
     {
      ...
      if(command == CCBSN_COMMAND_NEW_CYCLE_OFF &&
         g_startupCycleBarrierActive)
        {
         ...
         g_startupPolicyHoldUntilTick = GetTickCount64() +
                                        (ulong)InpStartupPolicyHoldSeconds * 1000;
...
string BuildMonitorStatusJson(const ulong sequence)
  {
   ...
   long startupHoldRemainingSeconds = 0;
   ulong monitorNowTick = GetTickCount64();
   if(g_startupPolicyHoldUntilTick > monitorNowTick)
      startupHoldRemainingSeconds = (long)MathCeil(
         (double)(g_startupPolicyHoldUntilTick - monitorNowTick) / 1000.0);
   ...
   json += "  \"ea_version\": \"4.1.0\",\n";
   json += "  \"policy_version\": " + JsonString(POLICY_VERSION) + ",\n";
   json += "  \"symbol\": " + JsonString(_Symbol) + ",\n";
   json += "  \"effective_quote_digits\": " + IntegerToString(g_effectiveQuoteDigits) + ",\n";
   ...
   json += "  \"expected_account_login\": " + IntegerToString(InpExpectedAccountLogin) + ",\n";
   json += "  \"expected_account_server\": " + JsonString(InpExpectedAccountServer) + ",\n";
   json += "  \"identity_valid\": " + JsonBool(g_identityValid && !g_identityViolationLatched) + ",\n";
   json += "  \"integrated_runtime_guard\": true,\n";
   ...
   json += "  \"startup_policy_hold_seconds_remaining\": " + IntegerToString(startupHoldRemainingSeconds) + ",\n";
...
int OnInit()
  {
   g_identityViolationLatched = false;
   ...
   if(!g_configurationValid)
     {
      ...
      g_objectPrefix = "CC_CONTROLLER_M5_V41." + IntegerToString(ChartID()) + ".";
      EventSetMillisecondTimer(InpTimerMilliseconds);
      Comment("CC Controller M5 Ver4.1 CONFIG SAFE MODE\n",
