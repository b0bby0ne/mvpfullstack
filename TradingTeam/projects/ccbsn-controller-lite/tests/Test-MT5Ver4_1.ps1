...
param(
    ...
    [string] $BuildBaseName = 'CC_Controller_M5_Ver4_1',
    [string] $ExpectedMqlVersion = '4.100',
    [string] $ExpectedPolicyId = 'cc-controller-m5-ver4.1',
    [string] $ExpectedPolicyVersion = '4.1.0-integrated',
    [string] $ExpectedObjectNamespace = 'CC_CONTROLLER_M5_V41.',
    [string] $ExpectedAuditFile = 'CC_Controller_M5_Ver4_1_Events.csv',
    [string] $ExpectedMonitorFile = 'cc_controller_m5_ver4_1_status.json',
    [int] $ExpectedStartupHoldMilliseconds = 70000,
    [string] $ExpectedTransportHash = '1DE2E01272BA4BE2D215E08FF9863C8EE349CBC566D71730145CABA16ABD91AB',
    [switch] $SkipCompile
)
...
if ([string]::IsNullOrWhiteSpace($SourcePath)) {
    $SourcePath = Join-Path $projectRoot 'src\mt5\CC_Controller_M5_Ver4_1.mq5'
}
...
Assert-SourcePattern 'Integrated control default' 'InpControlMode\s*=\s*CCBSN_CONTROL_ENABLED;'
...
Assert-SourcePattern 'M5 monitor schema' 'MONITOR_SCHEMA_VERSION\s*=\s*"ccbsn-lite-monitor-status\.v2"'
Assert-SourcePattern 'M5 monitor decision time' '\\"last_m5_decision_server\\"'
Assert-SourcePattern 'Quote digits auto-detect default' (
    'InpAutoDetectQuoteDigits\s*=\s*true;[\s\S]*?' +
    'g_effectiveQuoteDigits\s*=\s*InpAutoDetectQuoteDigits[\s\S]*?_Digits')
Assert-SourcePattern 'Explicit account login pin' (
    'InpExpectedAccountLogin[\s\S]*?actualLogin\s*!=\s*InpExpectedAccountLogin')
Assert-SourcePattern 'Exact account server pin' (
    'InpExpectedAccountServer[\s\S]*?actualServer\s*!=\s*InpExpectedAccountServer')
Assert-SourcePattern 'Force sync integrated default' (
    'InpForceSyncOnInit\s*=\s*true;')
Assert-SourcePattern 'Monitor integrated default' (
    'InpEnableExternalMonitor\s*=\s*true;')
Assert-SourcePattern 'Runtime identity latch' (
    'EnforceRuntimeIdentity\(\)[\s\S]*?g_identityViolationLatched\s*=\s*true;')
Assert-SourcePattern 'Identity mismatch forces barrier' (
    'EnforceRuntimeIdentity\(\)[\s\S]*?g_startupCycleBarrierActive\s*=\s*true;[\s\S]*?IDENTITY_MISMATCH')
Assert-SourcePattern 'Identity monitor fields' (
    'expected_account_login[\s\S]*?expected_account_server[\s\S]*?identity_valid[\s\S]*?integrated_runtime_guard')
Assert-SourcePattern 'Identity dashboard row' (
    'Identity: %I64d@%s[\s\S]*?Guard: %s \| Digits=%d')
Assert-SourcePattern 'Runtime identity checked on tick' (
    'void OnTick\(\)[\s\S]*?EnforceRuntimeIdentity\(\)')
Assert-SourcePattern 'Runtime identity checked on trade' (
    'void OnTradeTransaction[\s\S]*?EnforceRuntimeIdentity\(\)')
Assert-SourcePattern 'Runtime identity checked on timer' (
    'void OnTimer\(\)[\s\S]*?EnforceRuntimeIdentity\(\)')
...
if ($ExpectedStartupHoldMilliseconds -gt 0) {
    $expectedStartupHoldSeconds = [int]($ExpectedStartupHoldMilliseconds / 1000)
    Assert-SourcePattern 'Integrated startup policy hold input' (
        'InpStartupPolicyHoldSeconds\s*=\s*' +
        $expectedStartupHoldSeconds + ';')
...
Write-Host '  Runtime identity pin: account + server'
