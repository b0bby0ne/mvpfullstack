...
$localReleaseScript = Join-Path $PSScriptRoot 'Start-CCBSNSupervised-Ver4_0_1.ps1'
$scriptPath = if (Test-Path -LiteralPath $localReleaseScript) {
    $localReleaseScript
}
else {
    Join-Path $projectRoot 'tools\Start-CCBSNSupervised-Ver4_0_1.ps1'
}
...
$contracts = @(
    @{ Name='Supervisor version 4.0.1'; Pattern="supervisorVersion\s*=\s*'4\.0\.1'" },
