...
def main() -> int:
    ...
    while True:
        ...
        if not args.skip_price_agent:
            run_script("TradingTeam/tools/run_price_agent.py", ["--max-iterations", "1"])
            
        # 2. H4 Scanners (Agent 2)
        # Note: scan_h4_swing_setups.py scans crypto and cfd by default
        run_script("TradingTeam/tools/scan_h4_swing_setups.py", [])
        
        # 3. 1D Scanner (Agent 2 - VN Stock)
        run_script("TradingTeam/tools/scan_vn_1d_swing_setups.py", [])
        
        # 4. Telegram Alerts
        run_script(
            "TradingTeam/tools/process_telegram_alerts.py",
            [
